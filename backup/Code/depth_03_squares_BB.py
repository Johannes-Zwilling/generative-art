...
###############################################################
        ...
        for coordinate, pixel in zip(coordinates, pixels):
            ...
            #cv2.circle(img, tuple(coordinate), int(pixel * dotSizer), (156, 71, 96), -1, lineType = cv2.LINE_AA)
            cv2.rectangle(img, tuple(coordinate), tuple([int(tuple(coordinate)[0]+ (pixel * dotSizer)), int(tuple(coordinate)[1]+ (pixel * dotSizer))]), (156 + colorMod, 71 + colorMod, 96 + colorMod), -1)
